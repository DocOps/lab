...
* **Scripts**: The `labdev:run:scripts` task searches `scripts/` (local) before `scripts/.vendor/docopslab/` (synced).
* **Configs**: Linter configurations are generally merged, with the local `.config/` version taking precedence over the bundled vendor versions.
