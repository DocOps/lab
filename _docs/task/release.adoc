...
- [ ] CI builds and tests pass on `dev/{tok_majmin}`.
...
bundle exec rhx {tok_majmin}.{tok_patch} --md docs/release/{tok_majmin}.{tok_patch}.md
....
...
Or else use the GitHub web interface to manually register the release, and copy/paste the contents of `docs/release/{tok_majmin}.{tok_patch}.md` into the release notes field.
// end::step-announce[]
