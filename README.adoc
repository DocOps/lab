...
:this_proj_src_agent_docs_url: {this_proj_src_www_url}/blob/agent-docs
...
// end::general[]
// tag::projects[]
:box-desc: pass:q[A Docker-containerized environment and shell script for reducing the complexity of setting up “developer tools”. Non-developers can run a single command (`docksh run`) and instantly access whole runtimes and specialized documentation tools in a pre-configured shell environment.]
...
See the website sourced in this repo for the best overview of DocOps Lab and its projects: link:{docopslab_www_url}[docopslab.org].


[[repository]]
== This Repository

This codebase (`DocOps/lab`) contains the Jekyll-based website for DocOps Lab, served at {docopslab_www_url}[docopslab.org].
...
* a `feeds/` directory contains generated RSS and OPML feeds for the DocOps Lab Blog and a collection of third-party streams sourced in `_data/tech-docs-serials.yml`

[[the-site]]
=== The Site

The site has a link:{docopslab_www_url}[landing page] which leads mainly to a showcasing of link:{docopslab_www_url}/projects/[*DocOps Lab projects*] with a modern, professional interface featuring project cards, dependency visualization, and custom DocOps-themed icons.

It also includes link:{docopslab_www_url}/docs/[*technical documentation*] that is common across projects, as well as a link:{docopslab_www_url}/blog/[*blog*] for updates and topical writings.
...
* Vale styles package (`DocOpsLabStyles.zip`): link:{this_proj_src_www_url}/blob/main/artifacts/vale/DocOpsLabStyles.zip[GitHub repo assets]
...
* Agent docs library: link:{this_proj_src_www_url}/blob/agent-docs/agent/docs/[`agent/docs/` on `agent-docs` branch]
* Assets for the `docopslab-dev` tool are managed and published from the `DocOps/lab` (this) repository root (see <<library-publish>>).
...
It does _not yet_ use the link:{docopslab_www_url}/projects#jekyll-asciidoc-ui[jekyll-asciidoc-ui] or link:{docopslab_www_url}/projects/#jekyll-asciidoc-ext[jekyll-asciidoc-ext] plugins, which are still on the DocOps Lab roadmap.
...
If you want to report a bug or propose a fix (including a content correction):

* use the link:{this_proj_src_www_url}/issues[issues system] and post about it; and/or
...
See link:{docopslab_www_url}/docs/lab-dev-setup/[Devtool Setup] for details.
...
[[developing-docopslab-dev]]
=== Developing the docopslab-dev Gem
...
[.prompt]
 bundle exec rake gemdo:push:library:stage

.Generate `.library/catalog.json` catalog with SHA256 checksums
[.prompt]
 bundle exec rake gemdo:push:library:generate_catalog

.Force-push `.library/` to the `labdev-library` branch on GitHub
[.prompt]
 bundle exec rake gemdo:push:library:remote
...
Content includes anything sourced under `_docs/`, `_blog/`, and `_data/`.
