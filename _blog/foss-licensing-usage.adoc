...
:page-tags: ["open-source", "policy", "projects"]
...
The *code* is released on an interactive platform (link:{docopslab_src_www_url}[GitHub]) so you can investigate it, extend it, or help fix and improve it.
...
However, this does not mean you must modify or even look at the source code to make use of our software in the vast majority of cases.

In fact, if you know you're not going to "`hack`" our source code, skip to <<make-foss-work-for-you>>.
The next section is about the distinction between forking and modding open source software vs just installing and using it as intended by its provider.

[[modified-vs-packaged]]
=== Modified vs Packaged

There are a few ways to take advantage of DocOps Lab software and content.
Much of the following will apply to any similar open source runtime applications.

The most common way would be as a compiled-and-packaged dependency in your own project, perhaps as a Ruby gem or a Docker image.
...
Source code can be modified and used in many ways, including copied and pasted snippets, or even just as inspiration for your own code.
But of course you can also make a wholesale copy, modify it, and rebuild and package it for yourself, then cite that dependency in your `Gemfile` or `Dockerfile` or `Rakefile`, for instance.
...
Most runtime tools can be configured with local files that stablish a baseline of settings and behaviors.
...
DocOps Lab prefers to use YAML-based DSLs wherever possible, as they are easy to read and write, and YAML is widely supported.
...
Most of the engineering shops I've worked at disallow incorporating unlicensed code into published software, even if the developer obviously intended to share it.
...
MIT License is totally permissive; it should not trouble anyone's manager or CTO or Legal Department, and that's that.
...
CC BY indicates a requirement to note what changes are made if you redistribute the so-licensed content, so for stuff like Docs-as-Code School or link:{docopslab_src_www_url}[the main (this) DocOpsLab.org] website.
...
You may even appoint a different license to govern your changes/additions to the software.